...
AllSubData<-cbind(AllSubData,MaxActivation,MaxChoice,Accuracy,PropActivation)
...
AllMeans<-aggregate(MaxActivation~Type*PC*Congruency,AllSubData[AllSubData$Accuracy==TRUE,],mean)
...
AllMeans<-aggregate(PropActivation~Type*PC*Congruency,AllSubData[AllSubData$Accuracy==TRUE & AllSubData$trial>47,],mean)
...
AllMeans<-aggregate(PropActivation~trial*Congruency,AllSubData[AllSubData$Accuracy==TRUE,],mean)

ggplot(AllMeans,aes(x=trial, y=PropActivation, group=Congruency))+
  geom_line()+
  geom_point(aes(shape=Congruency))+
  theme_classic(base_size=12)+theme(
    legend.position="top",
